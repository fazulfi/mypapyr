...
The current codebase is an engineering foundation, and the merged product-level controls for the five-tool server path add strict admission validation, fail-closed ClamAV threat scanning wired into every tool router, a PDF sanitizer that refuses unsanitizable input, bounded worker execution, and deterministic cleanup. These controls are merged and deployed to production on 2026-08-17.

The Phase 6 enterprise controls are implemented, tested, and deployed (backend release p6-complete-1786951216, frontend release p6-ads-all-1786954951, 2026-08-17):
...
- **Advertising (PT-02):** reserved-dimension slots only, one symmetric ad slot per page (all-pages policy, owner decision 2026-08-17) — box-300x250 on the homepage and on tool pages after the result phase, banner-468x60 on supporting pages — delivery gated by Do Not Track / Global Privacy Control, house-promo fallback on provider failure, and never beside the Download control.
