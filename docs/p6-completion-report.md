...
- **Date**: 2026-08-17
- **Branch**: `feat/full-p6-enterprise-completion` @ `bce4932`
- **PR**: [fazulfi/mypapyr#46](https://github.com/fazulfi/mypapyr/pull/46) — CI 20/20 green (run 32002627224)
- **Deployed releases**: backend `p6-complete-1786951216` and frontend `p6-ads-all-1786954951` (BUILD_ID `HHzujraVbxQa5Q0LcI4dZ`) on faiz-prod (82.25.62.204), active 2026-08-17.
...
| WS-3 | Ads: one reserved-dimension unit per page (ADR-03), explicit phase/immediate guard (G12), house-promo fallback (ADR-04), `e2e/ad-behavior.spec.ts` (presence/absence/DNT-GPC/fallback), a11y E2E isolated from third-party ad network. All-pages policy applied (2026-08-17): one symmetric slot on every page — box-300x250 on the homepage (immediate) and tool pages (result-phase gated), banner-468x60 on the 8 supporting pages | ✅ |
...
| WS-6 | Docs reconciliation: roadmap self-contradiction removed, README legend + claims, product/architecture specs, AGENTS.md facts (9 routers, proxy.ts, 20 CI jobs), test counts, ADR-03/05 claims alignment | ✅ |
...
- Frontend activated via symlink + systemd unit `WorkingDirectory` update (unit file hardcodes the release path — not symlink-resolved); process cwd verified; BUILD_ID `HHzujraVbxQa5Q0LcI4dZ`.
- `nginx -t` passes (warnings are pre-existing shared-host vhost overlap).
- Public verification (Cloudflare): `/` → 307 → `/en`; `/en` `/es` `/id` → 200; `api.mypapyr.com/health` + `/health/ready` + `/api/v1/capabilities` → 200; hreflang `en/es/id/x-default` → budgezen.com; canonical → budgezen.com/en; sitemap 42 URLs; ad-slot marker present on `/en`, `/en/privacy`, and `/en/status` (all-pages policy); contact copy accurate.
