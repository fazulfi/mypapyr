...
Papyr is designed as a browser-first PDF platform with an explicit, bounded server-processing path. The repository currently implements the frontend foundation (including the shared trilingual shell), the backend service foundation with its API, queue, and storage contracts, deployment templates, and CI foundation described below. Native PDF processing, the upload and enqueue endpoints, and the concrete threat scanner remain later phases; the five PDF workflows are specified but not yet implemented.
...
- A Redis-backed backend foundation: a minimal-metadata task store, a durable Streams queue with queue caps and fair-use controls, a one-worker processing loop, an R2 client with opaque-key storage and presigned downloads, and a cleanup coordinator that enforces the hard one-hour (3,600-second) retention maximum. Validation, sanitizer, and threat-classification prerequisites are implemented; the concrete threat scanner is a later phase behind a defined protocol seam.
...
The versioned backend contracts below are implemented and covered by unit and integration tests. They describe the current foundation, not the full launch service: upload and enqueue endpoints, tool execution, and the concrete threat scanner are later phases.
...
- **Sanitizer and classification prerequisites.** Typed file validation runs before classification; classification is fail-closed — blocked before sanitize, never downgraded. The PDF sanitizer removes JavaScript, external access, and attachments, verifies the result, and refuses encrypted, malformed, or unsanitizable files. The scanner client that powers threat classification is a later phase behind the defined protocol seam.
