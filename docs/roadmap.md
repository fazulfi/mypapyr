...
The Phase 6 baseline is merged to `main` (PR #24) and deployed to production on 2026-08-15 (release 1767ca8) on the VPS backend (mypapyr.com / api.mypapyr.com) and the Vercel frontend (budgezen.com). The follow-up branch `feat/full-p6-enterprise-completion` adds PT-04 merge wiring, ad-placement E2E, SEO, and documentation reconciliation; it is not yet merged to `main`. Test gates measured on this branch: 780 frontend tests across 52 files (statements 91.27%, branches 86.06%, functions 91.71%, lines 93.10%) and 1360 backend tests (44 opt-in Redis integration tests skipped) with ruff and mypy strict clean.
...
Unit-test gating: frontend coverage measured on this branch at 91.27% statements, 86.06% branches, 91.71% functions, and 93.10% lines, meeting the 80% CI thresholds (branches threshold is 80, raised from the pre-existing 74 baseline); backend coverage measured at 89% (5393 statements) with ruff and mypy strict clean on the branch.

## Specified launch catalogue

The five-tool catalogue below is specified in the product specification and implemented; the Phase 5/6 baseline is deployed to production as described above, with the follow-up branch work tracked at the top of this document.
...
- Completion of the follow-up branch's deploy skeletons and the separately authorized production release of the remaining topology gaps (nginx `__SET_ME__` image/vhost, worker image digest publication, full Compose project activation), including the R2 lifecycle application and rollback drills.
