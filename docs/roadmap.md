...
## Merged: Phase 8 SEO and URL migration

P8 implements the three SEO workstreams without changing application business logic or deployment files. Merged to `main` via the Phase 9 squash (PR #49 → `31af3694`, 2026-08-20); the separate P8 PR was closed as superseded.
...
## Merged: Phase 9 Content, Legal & Blog

P9 is merged to `main` (PR #49 → `31af3694`, 2026-08-20) and deployed to the VPS frontend as release `p9-da6e94e` (BUILD_ID `2nIWv0nNRIdkFQbh92Ueh`).
...
Status: **Merged to `main`** (PR #49, squash `31af3694`); deployed to the VPS frontend as release `p9-da6e94e` (BUILD_ID `2nIWv0nNRIdkFQbh92Ueh`). External indexing, ranking, and search-engine presence remain NOT_VERIFIED.

## Merged: P7 enterprise operations (monitoring, status, alerts, backup)

The Phase 7 operations work is merged to `main` (PR #47 → `d91dec5`, 2026-08-21). It implements privacy-safe ops artifacts, contracts, and guards without touching the backend monitor schema or the frontend ad contract. Local verdicts (guards, unit tests, E2E, CI) are source-level evidence; production activation is owner-gated. The VPS host identity is resolved to `root@82.25.62.204` with the application directory `/opt/mypapyr` and service account `mypapyr`.
...
- **Owner gates (not provisioned, not executed)** — monitoring provider/threshold approval, Telegram bot/chat provisioning, backup S3 credentials and restic password, R-13 retention approval (documented placeholder families daily 7 / weekly 4 / monthly 12 / yearly 3). The conflicting VPS host targets from the P7 branch are resolved: the production host is `root@82.25.62.204` with the application at `/opt/mypapyr`.
- **Deployment status** — the P7 code is merged to `main` (PR #47 → `d91dec5`); production activation of monitoring, Telegram relay, and restic remains owner-gated pending credential provisioning and host-state confirmation (R-12/R-26). There is no live Telegram relay, no live restic repository, and no restore drill run against production (2026-08-17). Deployment follows the no-CD policy: PR and CI first, then gated runtime verification only with all gates cleared.

## In PR: Phase 10 pre-launch verification and launch gate

Phase 10 is implemented on branch `feat/phase-10-pre-launch-verification` and gated behind PR + CI (no-CD policy). It builds the five-tool trilingual launch gate and the coordinated production-activation toolkit (plan `docs/superpowers/plans/2026-07-31-papyr-rebuild-implementation-plan.md`, §Phase 10). Gate exit requires the launch gate to pass and the owner to authorize launch (G-3) — direct production activation without a beta label (DEC-096, DEC-140).

- **VL-01 five-tool trilingual E2E gate** — `frontend/e2e/five-tools.spec.ts` + `frontend/e2e/helpers.ts` covering the happy path, auto-download and manual-download fallback, routing transitions, error states, cancellation contract (absent on queued), refresh/session-resume contract, and reset across 5 tools × EN/ES/ID (DEC-027/029/068/072). Passes in both Chromium projects.
- **VL-02 accessibility program** — `frontend/axe-config.js` (WCAG 2.2 A/AA tags, `target-size` rule enabled) + `frontend/e2e/a11y/a11y-pages.spec.ts` (13 routes, ad-network isolation, SkipLink and keyboard focus-trap checks) + `docs/accessibility/exceptions-register.md` (DEC-062). Pre-existing violations fixed in `src` (nested `<main>` landmarks in the five tool pages; low-contrast `text-slate-400/500` and accent-on-tinted combos across 16 files); the axe gate now passes with zero violations.
- **VL-03 rendered visual verification** — `frontend/e2e/visual.spec.ts` (routes × viewports 375/768/1280/1440, overflow, token colors, accent, `main#main-content`, CLS < 0.1, screenshots under `frontend/test-results/visual/`), `scripts/check-contrast.sh` + `scripts/test-check-contrast.sh` (fail-closed WCAG contrast of documented token combos, DEC-143 / UX §21.12 / §21.19).
- **VL-04 Core Web Vitals + performance** — `frontend/lighthouserc.js` (performance/accessibility/best-practices/SEO ≥ 90; FCP ≤ 1800 ms, LCP ≤ 2500 ms, CLS ≤ 0.1, TBT ≤ 300 ms) + `docs/verification/performance.md` (R-27 per DEC-200/201, ad-slot stability DEC-018).
- **VL-05 pre-launch smoke, rollback readiness, activation checklist** — `docs/verification/launch-checklist.md`, `docs/verification/smoke.md`, `scripts/check-launch.sh` (offline preflight, HTTP smoke, rollback-preflight evidence modes; read-only, privacy-safe output) + `scripts/test-check-launch.sh` (DEC-140/178/160/177). `deploy/runbook-vps.md` rollback now restores all three images (API/workers/ClamAV), documents the production `3016:3000` override mapping and frontend BUILD_ID/nginx-cutover rollback; `docs/upgrade.md` and `scout-configs.md` environment-path references reconciled to `/opt/mypapyr/production/.env`.
- **CI** — four new jobs (`frontend-a11y`, `frontend-visual`, `frontend-perf`, `qa-launch`) raise the total from 25 to **29** (28 on pushes to main).

Status: **In PR on branch `feat/phase-10-pre-launch-verification`**; all local gates pass (unit ≥ 80%, lint, typecheck, build, targeted Playwright runs green in both projects). The launch gate itself (`scripts/check-launch.sh` against production) and the G-3 owner authorization remain pending.
