...
- **Release boundary:** the VPS-only legacy-host cutover is deployed and verified: `mypapyr.com` and `www.mypapyr.com` return host-level 308 responses to `https://budgezen.com$request_uri`; the exact nginx source diff and timestamped rollback backup are recorded in the operator evidence. `budgezen.com` remained untouched and healthy. The branch's application release is still not merged/deployed; external indexing, ranking, R-25 traffic, and any SEO SLO remain NOT_VERIFIED. Rollback is the preserved `/etc/nginx/sites-available/mypapyr.bak-cutover-<UTC timestamp>` file, restored only after a successful `nginx -t` check.
