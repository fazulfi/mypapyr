...
The Phase 6 baseline is merged to `main` (PR #24) and deployed to production on 2026-08-15 (release 1767ca8) on the VPS backend (mypapyr.com / api.mypapyr.com) and the Vercel frontend (budgezen.com). The Phase 6 enterprise completion (PT-04 merge wiring and ad-placement E2E) shipped via PR #46 and is deployed as backend release p6-complete-1786951216 and frontend release p6-ads-all-1786954951 (2026-08-17). P8 SEO implementation is **In branch** on `feat/full-p8-seo-url-migration`; it has no commit, PR, release, or deployment. Host canonical alignment, deployment host, R-25 traffic, external indexing, ranking, and any SEO SLO are therefore NOT_VERIFIED. Test gates: 780 frontend tests across 52 files (statements 91.27%, branches 86.15%, functions 91.71%, lines 93.10%) and 1360 backend tests (44 opt-in Redis integration tests skipped) with ruff and mypy strict clean.
...
- **SEO-01 governance:** `docs/seo/slug-table.md` records the 42 indexable localized URLs (5 tools × 3 locales, 8 supporting routes × 3 locales, and 3 locale home routes); `docs/seo/legacy-url-inventory.md` records exactly 15 legacy locale-less paths with 5 × 301, 8 × 410, and 2 × 307 dispositions. `scripts/check-seo-inventory.sh` and its self-test fail closed on drift.
...
- **SEO-03 metadata:** `SEO_BASE_URL` is currently pinned in code to `https://budgezen.com`, with per-route canonical and `en`/`es`/`id`/`x-default` alternates, 42 sitemap entries, robots sitemap alignment, explicit non-indexable exclusions, and deterministic committed `LAST_MODIFIED = "2026-08-18"`. The host decision remains NOT_VERIFIED because deployment documentation also identifies `mypapyr.com` as the VPS frontend host; no canonical host, deployment host, indexing, ranking, traffic, or certification claim is made.
- **Release boundary:** no P8 deployment, commit, push, or PR was performed in this work. A future release must verify both candidate hosts, `/sitemap.xml`, `/robots.txt`, the 301/410/307 responses, canonical/hreflang output, and the intended build identifier before activation. Rollback is the existing pointer/digest rollback described in `docs/release-checklist.md` and `deploy/runbook-vps.md`; no rollback drill is claimed here.
...
- Full legal, support, and status content and functionality beyond the route shells and the Phase 6 contact form (whose email delivery awaits owner-provisioned Cloudflare Email Sending credentials), and the blog publishing programme.
