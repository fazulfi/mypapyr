...
export const BASE_URL = SEO_BASE_URL;

// P8-F / SEO-03 — real lastmod: a deterministic, committed content-change
// constant (NOT build-time `new Date()`). Every sitemap entry shares this date
// because all routes' canonical metadata/hreflang were (re)generated by the
// SEO-03 work; a route's timestamp moves only when its content changes.
export const LAST_MODIFIED = "2026-08-18";
...
export default function sitemap(): MetadataRoute.Sitemap {
  return SITEMAP_GROUPS.flatMap((entry) => locales.map((locale) => buildEntry(entry, locale)));
}
