...
export async function generateMetadata({ params }: ToolLayoutProps): Promise<Metadata> {
  ...
  return {
    title: copy.tools.compress.title,
    description: copy.tools.compress.description,
    alternates: alternateLinks(locale as Locale, tool.hrefs),
  };
}

export default function CompressPdfLayout({ children }: ToolLayoutProps): ReactNode {
  return children;
}
