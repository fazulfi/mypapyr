...
| Blog publishing programme: 15 trilingual MDX articles (5 topics × en/es/id), listing, article pages, sitemap entries, and content CI gates | Deployed |
...
| Shared upload, progress, error, and download experience | Deployed |
| Full legal, support, and status content and functionality | Deployed |
...
| Five-tool trilingual E2E gate, WCAG 2.2 accessibility program, visual and Core Web Vitals verification, pre-launch smoke and rollback toolkit | In PR (Phase 10) |
...
The worker loop, cleanup loop, production monitor, and R2 lifecycle policy gate live under `backend/app/` and are active in production as compose services (cleanup, monitor) per the roadmap. From `backend/` with dependencies installed:
