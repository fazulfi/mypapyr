...
- **Typecheck hardening (ADR-09)** — fixed 5 pre-existing `tsc --noEmit` errors and added a new `frontend-typecheck` CI job (CI is now 23 jobs, 22 on pushes to main).
...
- **CI** — 23 jobs (22 on pushes to main, incl. `frontend-typecheck` and the P7 ops guards `qa-monitoring`, `qa-telegram-relay`, `qa-backup`); branch protection requires 7 status checks.
